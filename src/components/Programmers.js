...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    const person = activeName - 1
    const name = names[person].name
    return name;
  ...
  const style = {
    ...
    color: activeName<=0 ? 'royalblue' : 'gold'
  };
      ...
      <div className='programmers'>
        {
          ...
          names.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { /* in here set the featured id to be dev.id */  setActiveName(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
